...
import persistencia.conexion.ArchivoConfiguracion;
import persistencia.conexion.Conexion;
import presentacion.reportes.ReporteAgenda;
import presentacion.vista.VentanaBD;
import presentacion.vista.VentanaLocalidad;
import presentacion.vista.VentanaPersona;
import presentacion.vista.VentanaTipoDeContacto;
import presentacion.vista.Vista;
import presentacion.vista.VentanaInicioBD;
...
public class Controlador implements ActionListener, MouseListener 
{
		...
		private VentanaInicioBD ventanaInicioBD;
		private VentanaBD ventanaBD;
		private Agenda agenda;
		private Validador validador;
		private boolean seAgregaNueva;
		private ArchivoConfiguracion archivo;
		
		public Controlador(Vista vista, Agenda agenda)
		{
			this.vista = vista;
			this.vista.getmntmConfigurarAcceso().addActionListener(this);
			...
			this.archivo = new ArchivoConfiguracion();
			this.agenda = agenda;
		...
		{
			if(!archivo.existeArchivoConfiguracion())
			{
				abrirventanaInicioBD();
			}
			else
			{
				try 
				...
				{
					Dialogo.error("Acceso denegado, los datos de la configuracion no son validos para la conexion", "Error de conexion");
					int confirmacion = Dialogo.confimarcion("Si deseas configurar el acceso haz click en 'Si', si deseas salir presiona 'No'.", "Configurar acceso");
					if(confirmacion == 0) { abrirVentanaBD(); }
					else { System.exit(0); }
				}
			}
		}
		...
		{		
			...
			else if (e.getSource() == this.vista.getmntmConfigurarAcceso()) 
			{
				abrirVentanaBD();
			}
			...
			else if(this.ventanaTipoDeContacto != null && e.getSource() == this.ventanaTipoDeContacto.getBtnAgregar()) 
			{
			...
			else if (this.ventanaTipoDeContacto != null && e.getSource() == this.ventanaTipoDeContacto.getBtnModificar()) 
			{
				if(esTipoDeContactoValido())
				{
			...
			else if(this.ventanaTipoDeContacto != null && e.getSource() == this.ventanaTipoDeContacto.getBtnEliminar()) 
			{
				borrarTipoDeContacto();
			}
			else if (this.ventanaInicioBD != null && e.getSource() == this.ventanaInicioBD.getBtnIniciar()) 
			{
				if (datosValidos()) 
				{
					crearArchivoDeConfiguracion();
					this.ventanaInicioBD.dispose();
					inicializar();
				}
			}
			else if (this.ventanaInicioBD != null && e.getSource() == this.ventanaInicioBD.getBtnCerrar()) 
			{
					this.ventanaInicioBD.dispose();
			}
			else if (this.ventanaBD != null && e.getSource() == this.ventanaBD.getbtnCancelar()) 
			{
				this.ventanaBD.dispose();
			}
			else if (this.ventanaBD != null && e.getSource() == this.ventanaBD.getbtnActualizar()) 
			{
				if(camposValidos())
				{
					modificarArchivo();
					this.ventanaBD.dispose();
					Dialogo.mensaje("La modificacion se realizo con exito", "Modificacion de acceso");
					Dialogo.mensaje("Para aplicar los cambios la aplicacion debe reiniciar", "Aviso de Reinicio");
					try {
		...
		private void abrirVentanaBD() 
		{
			this.ventanaBD = VentanaBD.getVentana(this);
			setFieldsVentanaBD();
		}

		private void setFieldsVentanaBD() 
		{
			setTxtField(this.ventanaBD.getCurrentHostname(), archivo.getProperty("jdbc.hostname"));
			setTxtField(this.ventanaBD.getCurrentPuerto(), archivo.getProperty("jdbc.puerto"));
			setTxtField(this.ventanaBD.getCurrentUsuario(), archivo.getProperty("jdbc.usuario"));
			setTxtField(this.ventanaBD.getCurrentPassword(), archivo.getProperty("jdbc.password"));
		}
		
		private void abrirventanaInicioBD()
		{
			this.ventanaInicioBD = VentanaInicioBD.getVentana(this);
		}
		
		//***************METODOS DE CONFIGURACION DE ACCESO A LA BD************************//
		
		private void modificarArchivo() 
		{
			String hostname = this.ventanaBD.getCurrentHostname().getText();
			String puerto = this.ventanaBD.getCurrentPuerto().getText();
			String usuario = this.ventanaBD.getCurrentUsuario().getText();
			String password = this.ventanaBD.getCurrentPassword().getText();
			archivo.modificar(hostname, puerto, usuario, password);
		}

		private void crearArchivoDeConfiguracion() 
		{
			String hostname = this.ventanaInicioBD.getTxtHostname().getText();
			String puerto = this.ventanaInicioBD.getTxtPuerto().getText();
			String usuario = this.ventanaInicioBD.getTxtUsuario().getText();
			String password = this.ventanaInicioBD.getTxtPassword().getText();
			archivo.crear(hostname, puerto, usuario, password);
		}
...
//*************************VALIDACIONES VENTANA PERSONA******************************//		
		...
		private boolean verificarCampos()
		{
			...
			{
				Dialogo.error("La altura debe ser un numero de hasta 4 digitos", "Error");
				camposValidos = false;
			}
			else if(!validador.pisoValido(piso))
			{
				Dialogo.error("Debe ser un numero de hasta 2 digitos", "Error");
				camposValidos = false;
			}
		...
		private boolean camposValidos()
		{
			...
			if(estaVacio(hostname)||estaVacio(puerto)||estaVacio(usuario)||estaVacio(password))
			{
				Dialogo.error("Por favor, ingrese los datos correctamente", "Error");
				camposValidos = false;
			}
			else
			{
